...
from mr_short.live.scanner import (Candidate, check_entry, evaluate_candidate,
                                   rank_table)
...
def build_watchlist(cfg) -> list:
    ...
    for _, row in selected.iterrows():
        ...
        out.append(Candidate(
            ...
            stop=trade.stop, target=trade.target, atr=float(row["atr14"]),
            prev_close=float(row["close"]),
            avg_daily_volume=float(row["turnover_cr"]) * 1e7 / float(row["close"]),
            qty=trade.qty, lots=trade.lots, lot_size=trade.lot_size,
        ))
    return out


def place_entry(c: Candidate, engine: OrderEngine, cfg, kite, state) -> None:
    inst = resolve(kite, c.symbol, cfg["entry"]["product"], c.lot_size)
    px = c.live.get("px", c.entry_level)
    if not engine.check_margin(inst, c.qty, px):
        c.status, c.drop_reason = "DROPPED", "margin check failed"
        return
    entry_id = engine.place_short_market(inst, c.qty, "1m breakdown confirmed")
    gtt_id = None
    if cfg["exits"].get("use_gtt", True) and cfg["entry"]["product"] == "NRML_FUT":
        gtt_id = engine.place_gtt_oco(inst, c.qty, c.stop, c.target, last_price=px)
    state["trades"].append({
        "symbol": c.symbol, "tradingsymbol": inst.tradingsymbol,
        "exchange": inst.exchange, "product": cfg["entry"]["product"],
        "qty": c.qty, "entry": px, "stop": c.stop, "target": c.target,
        "scan_day_low": round(c.trigger / (1 - cfg["entry"]["breakdown_buffer_pct"] / 100), 2),
        "risk_amount": round(c.qty * (c.stop - px), 0),
        "entry_order_id": entry_id, "sl_order_id": None,
        "target_order_id": None, "gtt_id": gtt_id, "status": "OPEN",
        "entry_date": dt.date.today().isoformat(), "sessions_held": 0,
        "plan_file": "live_scan", "entry_mode": "live_1m",
    })
    save_state(state)
    c.status = "ENTERED"
    log.info(f"*** ENTERED {c.symbol} x{c.qty} @ ~{px} "
...
def main():
    ...
    while True:
        now = now_ist()
        active = [c for c in watch if c.status in ("WATCH", "ARMED")]
        ...
        if now.time() >= start and not daily_loss_breached(kite, cfg):
            armed = sorted([c for c in active if c.status == "ARMED"],
                           key=lambda c: -c.live.get("score", 0))
            for c in armed:
                if capacity <= 0:
                    break
                if check_entry(c, bars.get(c.symbol), lcfg):
